...
class AnsiStripperTest {
    ...
    // --- NEW: cursor-forward sequences used by Claude CLI for word spacing ---

    @Test
    void convertsCursorForward1ToSpace() {
        // ESC[1C is cursor-forward-1, used between words in Claude CLI prompts
        assertEquals(" Accessing workspace:", AnsiStripper.strip("\u001B[1CAccessing\u001B[1Cworkspace:"));
    }

    @Test
    void convertsCursorForwardNToNSpaces() {
        // ESC[3C = cursor-forward-3 → 3 spaces
        assertEquals("   gap", AnsiStripper.strip("\u001B[3Cgap"));
    }

    @Test
    void convertsCursorForwardNoArgTo1Space() {
        // ESC[C with no number defaults to 1
        assertEquals(" word", AnsiStripper.strip("\u001B[Cword"));
    }
    ...
    void stripsClaudeTrustPromptReadably() {
        ...
        assertEquals(" Accessing workspace: /", AnsiStripper.strip(input));
    }
}
