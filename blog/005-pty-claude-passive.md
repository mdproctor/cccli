...
The right fix is WKWebView + xterm.js. That only works in a proper `.app` bundle — we've known this since Plan 2. The question now is what breaks when we actually build the bundle and try it.

---

**Next:** Package the binary as a `.app` bundle, wire in WKWebView + xterm.js, and find out what breaks.
